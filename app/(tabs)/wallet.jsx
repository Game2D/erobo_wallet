  ...
  return (
    <SafeAreaView>
      ...
      <View className="px-8 py-12 z-20">
        <View>
          <Text className="font-pmedium text-[20px]">Total Wallet Value</Text>
          <Text className="font-psemibold text-[32px]">$29,100.50</Text>
        </View>
        ...
        <View className="flex items-center mt-6">
          <View className="relative flex flex-row gap-8 bg-[#EBEFF1] rounded-full py-4 px-5 overflow-hidden">
            {items.map((item, index) => (
              ...
              >
                <Text 
                  className={`text-[14px] px-9 ${selected === index ? 'text-black font-psemibold' : 'text-gray-200 font-pmedium'}`}
                >
                  {item}
                </Text>
              </TouchableOpacity>
