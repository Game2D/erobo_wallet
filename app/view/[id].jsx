...
const ViewCrypto = () => {
  ...
  return (
    <SafeAreaView>
      <ScrollView className="bg-[#F3F5F6]">
      <Image 
        source={images.blur3}
        className="absolute top-0 left-0 w-full h-full z-10"
      />
      <View className="h-full flex py-12 z-20">
        <View className="px-8 flex flex-row justify-between items-center">
          ...
          <Text className="text-[14px] font-pmedium text-black">{item.label}</Text>
        ...
        <View className="px-8 flex items-center mt-6">
          <Text className="text-[28px] font-pmedium">${item.price}</Text>
          ...
          <View className="flex items-center mt-6">
            <View className="relative flex flex-row gap-8 bg-[#EBEFF1] rounded-full py-4 px-5 overflow-hidden">
              {time.map((item, index) => (
                ...
                >
                  <Text 
                    className={`text-[14px] px-2 ${selected === index ? 'text-black font-psemibold' : 'text-gray-200 font-pmedium'}`}
                  >
                    {item}
                  </Text>
                </TouchableOpacity>
        ...
        <View className="flex mt-8 bg-white rounded-t-[30px] p-8 h-full">
            <Text className="font-pmedium text-[20px] text-black">Market Statistics</Text>

            <View className="flex flex-row justify-between mt-4">
              <Text className="text-gray-200 font-pmedium text-[14px]">Market capitalization</Text>
              <Text className="text-[14px] font-psemibold">$231,233</Text>
            </View>

            <View className="flex flex-row justify-between mt-2">
              <Text className="text-gray-200 font-pmedium text-[14px]">Circulating Suply</Text>
              <Text className="text-[14px] font-psemibold">114.211 ETH</Text>
            </View>
        </View>
      </View>
